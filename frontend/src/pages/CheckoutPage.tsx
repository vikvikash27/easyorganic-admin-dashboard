...
const CheckoutPage: React.FC = () => {
  ...
  const [address, setAddress] = useState<Address>({
    fullName: '', // Start blank for flexibility (e.g., for gifts)
    street: '',
    city: '',
    state: '',
    zip: '',
        ...
        if (!response.ok) {
            ...
            try {
                ...
                if (errorText && errorText.includes('Cannot POST')) {
                    errorMessage = 'Server error: Endpoint not found. Please contact support.';
                } else {
    ...
    <div className="container mx-auto px-6 py-12">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-5 gap-12">
        <div className="lg:col-span-3 bg-white p-8 rounded-lg shadow-md">
            <h2 className="text-xl font-semibold mb-6">Enter Delivery Address</h2>
            <div className="space-y-4">
              <Input label="Full Name" id="fullName" value={address.fullName} onChange={handleInputChange} required placeholder="Recipient's Full Name" />
              <Input label="Street Address" id="street" value={address.street} onChange={handleInputChange} required placeholder="House No. & Street Name" />
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Input label="City" id="city" value={address.city} onChange={handleInputChange} required />
                <Input label="State / Province" id="state" value={address.state} onChange={handleInputChange} required />
              </div>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Input label="ZIP / Postal Code" id="zip" value={address.zip} onChange={handleInputChange} required />
                <Input label="Country" id="country" value={address.country} onChange={handleInputChange} disabled />
              </div>
              <Input label="Phone Number" id="phone" type="tel" value={address.phone} onChange={handleInputChange} required placeholder="Recipient's 10-digit phone number" />
              <div className="pt-4">
                <MapInput onLocationSelect={handleMapAddressSelect} />
              </div>
              <div className="pt-4">
                  <h3 className="text-lg font-medium text-slate-700 mb-2">Payment Method</h3>
                  <div className="flex gap-4">
                      <label className={`flex items-center p-3 border rounded-lg cursor-pointer ${paymentMethod === 'Card' ? 'border-brand-primary bg-indigo-50' : 'border-slate-300'}`}>
                          <input type="radio" name="paymentMethod" value="Card" checked={paymentMethod === 'Card'} onChange={() => setPaymentMethod('Card')} className="mr-2"/>
                          Card
                      </label>
                      <label className={`flex items-center p-3 border rounded-lg cursor-pointer ${paymentMethod === 'COD' ? 'border-brand-primary bg-indigo-50' : 'border-slate-300'}`}>
        ...
        <div className="lg:col-span-2">
            <div className="bg-white p-8 rounded-lg shadow-md sticky top-24">
                ...
                <Button type="submit" className="w-full mt-6" disabled={isLoading}>
                    {isLoading ? <SpinnerIcon className="mx-auto" /> : `Place Order (₹${cartTotal.toFixed(2)})`}
                </Button>
            </div>
        </div>
      </form>
