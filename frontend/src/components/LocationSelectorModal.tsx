  ...
  if (onConfirm) {
    ...
    useEffect(() => {
      if (isOpen && mapRef.current && window.google) {
        const center = pinnedLocation || { lat: 28.6139, lng: 77.209 };
          ...
          (error) => {
            ...
            switch (error.code) {
              case error.PERMISSION_DENIED:
                message =
                  "You denied location access. Please enable it in your browser settings.";
                break;
        ...
        >
          <div className="p-6 border-b border-slate-200 flex justify-between items-start gap-4">
            <div>
              ...
              <p className="text-slate-500 mt-1">
                Click or drag the marker to set your address.
              </p>
            </div>
          ...
          <div className="p-1 sm:p-2 md:p-6 flex-grow relative">
            <div
              ...
              className="rounded-lg bg-slate-200 flex items-center justify-center text-slate-500"
            >
              Loading Map...
            </div>
            {geolocationError && (
          ...
          <div className="p-4 bg-slate-50 rounded-b-2xl flex justify-end items-center gap-4">
            {pinnedLocation && (
              <p className="text-xs text-slate-600 mr-auto">
                Lat: {pinnedLocation.lat.toFixed(4)}, Lng:{" "}
            ...
            <Button onClick={handleConfirm} disabled={!pinnedLocation}>
              Confirm Location
            </Button>
          </div>
        </div>
      ...
      >
        <div className="p-6 border-b border-slate-200 flex justify-between items-start">
          <div>
            ...
            <p className="text-slate-500 mt-1">
              Provide your location to server you better
            </p>
          </div>
          <button
            onClick={onClose}
            className="text-slate-500 hover:text-slate-800 p-2 -mr-2 -mt-2"
          >
            <CloseIcon className="w-6 h-6" />
          </button>
        </div>
